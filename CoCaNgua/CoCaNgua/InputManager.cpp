...
void Mouse::processMouse( int button, int state, int x, int y )
{
  if (Graphic::inst().getAppScene() == APP_GAME)
  {
    ...
    if 	((button == GLUT_LEFT_BUTTON) && (state == GLUT_DOWN))
    {
      ...
      {
        mousedw(x, y, button);
        createRay(x, Graphic::inst().screenHeight-y);
      }
    } 
  }
...
void Mouse::createRay( int mouse_x, int mouse_y )
{
  int window_y = (Graphic::inst().screenHeight - mouse_y) - Graphic::inst().screenHeight/2;
